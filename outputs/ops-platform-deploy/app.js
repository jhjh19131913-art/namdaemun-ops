...
function loadCloudConfig() {
  try {
    ...
    state.cloud.changeVersion = Number(saved.changeVersion) || 0;
    state.cloud.savedVersion = Number(saved.savedVersion) || 0;
...
function applyDefaultCloudConfig() {
  ...
  state.cloud.changeVersion = 0;
  state.cloud.savedVersion = 0;
...
function saveCloudConfig() {
  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      changeVersion: state.cloud.changeVersion,
      savedVersion: state.cloud.savedVersion,
...
function armCloudPush(delay = CLOUD_PUSH_DELAY_MS) {
...
function markCloudPushSucceeded(pushedVersion = state.cloud.changeVersion) {
  state.cloud.savedVersion = Math.max(state.cloud.savedVersion, pushedVersion);
  const hasNewerChanges = state.cloud.changeVersion > pushedVersion;
  state.cloud.hasPendingChanges = hasNewerChanges;
  state.cloud.pendingSince = hasNewerChanges ? state.cloud.pendingSince || new Date().toISOString() : null;
  state.cloud.lastErrorMessage = "";
  state.cloud.lastErrorAt = null;
  state.cloud.retryDelayMs = CLOUD_RETRY_DELAY_MS;
  ...
  saveCloudConfig();
  renderCloudSettings();
  if (hasNewerChanges) armCloudPush(0);
}
...
async function pushPendingCloudState(options = {}) {
  ...
  clearTimeout(state.cloud.pushTimer);
  const pushedVersion = state.cloud.changeVersion;
  state.cloud.isSyncing = true;
  renderCloudSettings();
  try {
    await Promise.all([
    ...
    markCloudPushSucceeded(pushedVersion);
...
async function syncCloud(options = {}) {
  ...
  if (state.cloud.hasPendingChanges && !options.skipPendingPush) {
    try {
      await pushPendingCloudState({ silent: true, force: true });
    } catch {
      if (!options.silent) showToast("새 변경사항 저장 후 다시 동기화합니다.");
      return;
    }
    if (state.cloud.hasPendingChanges) {
      if (!options.silent) showToast("새 변경사항을 먼저 저장하고 있습니다.");
      return;
    }
  }
  const syncVersion = state.cloud.changeVersion;
  state.cloud.isSyncing = true;
  renderCloudSettings();
  try {
    ...
    markCloudPushSucceeded(syncVersion);
